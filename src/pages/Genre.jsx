  ...
  return (
    <>
      ...
      <main className="mx-auto max-w-7xl px-4 pb-16 pt-12 sm:px-6 lg:px-8">
        <div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end"><div><p className="section-kicker"><Layers3 size={14} /> Browse by mood</p><h1 className="mt-3 font-display text-6xl leading-none text-white sm:text-8xl">{selectedGenre || 'Every genre'}</h1></div><Link to={routes.discover} className="inline-flex items-center gap-2 text-sm font-bold text-neutral-content hover:text-primary">Search the collection <ArrowUpRight size={16} /></Link></div>
        <div className="scrollbar-none mt-10 flex gap-2 overflow-x-auto border-b border-white/10 pb-5"><Link to={routes.genres} className={`shrink-0 rounded-full border px-4 py-2 text-xs font-bold transition ${!selectedSlug ? 'border-primary bg-primary text-primary-content' : 'border-white/10 bg-white/5 text-neutral-content hover:text-white'}`}>All genres</Link>{genres.map((genre) => <Link key={genre} to={routes.genre(genre)} className={`shrink-0 rounded-full border px-4 py-2 text-xs font-bold transition ${genreSlug(selectedGenre) === genreSlug(genre) ? 'border-primary bg-primary text-primary-content' : 'border-white/10 bg-white/5 text-neutral-content hover:text-white'}`}>{genre}</Link>)}</div>
